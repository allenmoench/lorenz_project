...
def integrate(state0, tendency_fn, dt, n_steps):
    ...
    for i in range(n_steps):
        trajectory[i+1] = euler_step(trajectory[i], tendency_fn, dt) # I changed this to euler_step(state, tendency_fn, dt) from euler_step(trajectory[i-1,:]) ...where do I put i-1?
    return trajectory
